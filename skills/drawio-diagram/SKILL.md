...
- `assets/erofs-image-layout.png` 整体布局 + 寻址（条带、字段指向区域、地址公式）
...
1. 判断图类型（架构 / 布局 / 流程 / 时序），列出元素：分层或分列、每个盒子、边界、箭头及其标签、需要标注的命令/配置。
2. 先定坐标（10px 网格），再写一个 `gen_<name>.py`：`sys.path.insert(0, '<skill>/scripts')` 后 `from dio import Diagram`，`save()` + `Diagram.export()`。
3. Read 导出的 PNG 逐项过「检查清单」，不合格改坐标重导。
4. 交付 .drawio 和 .png 路径，一两句说明。不写长篇解释。
...
布局图（磁盘格式、内存布局）：第一行是完整条带（盒子 10px 间距、按偏移顺序），下面各行用 `brace(up=True)` 从条带某段放大，`brace(up=False)` 把一组小块归并到下方的大块。

共同规则：同层平级组件等宽等距；主组件宽盒子放右、辅助放左；命令/配置用 `note` 放图的上方或两侧，`dotted` 引到对应盒子。

```python
d.text('Image\nLayout', 20, 40, 120, 70, bold=True)
sb = d.box('Superblock', 320, 40, 140, 70, 'orange', sub='@1024 · 128 B')
d.brace(170, 460, 125, up=True); d.text('Block 0 · 4 KiB', 240, 130, 150, 20, size=13)
n = d.note('dump.erofs -S image.erofs', 280, -30, 220, 30, 'grey'); d.dotted(n, sb, (.5, 1), (.5, 0))
```
