...
SB = [('magic', 'E0F5E1E2'), ('blkszbits', '12 → 4 KiB'), ('root_nid', 'root dir inode'), ('meta_blkaddr', 'metadata start'),
      ('inos', 'inode count'), ('xattr_blkaddr', 'shared xattr start'), ('blocks', 'fs size in blocks'), ('devt_slotoff', 'device table')]
cells = [d.box(t, 170 + i * 120, 40, 110, 60, 'orange', bold=False, sub=s) for i, (t, s) in enumerate(SB)]
n0 = d.note('dump.erofs -S image.erofs', 1160, 55, 210, 30, 'grey'); d.dotted(cells[7], n0, (1, .5), (0, .5))

d.text('Image\nLayout', 20, 200, 120, 70, bold=True)
sb = d.box('Superblock', 170, 200, 150, 70, 'orange', sub='@1024 · 128 B')
meta = d.box('Metadata Area', 330, 200, 340, 70, 'yellow', sub='inode slots · xattrs · tail data')
sx = d.box('Shared xattrs', 680, 200, 260, 70, 'yellow', sub='xattr_blkaddr · 4 B aligned')
data = d.box('Data Area', 950, 200, 280, 70, 'blue', sub='file & directory blocks')
dev = d.box('Device Table', 1240, 200, 150, 70, 'green', sub='extra devices · blobs')
d.dotted(cells[0], sb, (.5, 1), (0.367, 0))
d.edge(cells[2], meta, (.5, 1), (0.397, 0), 'root inode', offset=(40, 0))
d.edge(cells[3], meta, (.5, 1), (0.75, 0), 'start', offset=(26, 0))
d.edge(cells[5], sx, (.5, 1), (0.558, 0), 'start', offset=(26, 0))
d.edge(cells[7], dev, (.5, 1), (.5, 0), points=[(1065, 150), (1315, 150)])

d.text('Addressing', 20, 340, 120, 100, bold=True)
for i, t in enumerate(['nid 0', 'nid 1', 'nid 2']): d.box(t, 330 + i * 110, 340, 100, 60, 'yellow', bold=False, sub='32 B slot')
d.brace(330, 650, 325, up=True); d.text('addr = meta_blkaddr × 4 KiB + nid × 32', 330, 415, 340, 20, **F)
for i, t in enumerate(['id 0', 'id 1']): d.box(t, 680 + i * 110, 340, 100, 60, 'yellow', bold=False, sub='4 B unit')
d.brace(680, 890, 325, up=True); d.text('addr = xattr_blkaddr × 4 KiB + id × 4', 680, 415, 260, 20, **F)
for i, t in enumerate(['blk 0', 'blk 1', '...']): d.box(t, 950 + i * 90, 340, 80, 60, 'blue', bold=False)
d.brace(950, 1210, 325, up=True); d.text('addr = blkaddr × 4 KiB', 950, 415, 280, 20, **F)
out(d, 'erofs-image-layout')

# ================= 2. Regular file =================
d = Diagram()
d.text('Regular file\ninode slot', 20, 60, 120, 70, bold=True)
ino = d.box('erofs_inode', 170, 60, 220, 70, 'yellow', sub='compact 32 B · extended 64 B')
...
d.edge(ino, b0, (.5, 0), (.5, 0), 'i_u.raw_blkaddr · n full blocks in data area', points=[(280, 25), (1180, 25)], offset=(0, -6), lstyle='verticalAlign=bottom;')
d.brace(1130, 1560, 145, up=False); d.text('i_size = n × 4 KiB + len(tail)', 1130, 158, 430, 20, **F)
d.brace(170, 390, 145, up=False)
IN = [('i_format', 'ver · datalayout'), ('i_xattr_icount', 'xattr size / 4'), ('i_mode', 'S_IFREG · perms'), ('i_nlink', 'link count'),
      ('i_size', 'bytes'), ('i_ino', 'inode number'), ('i_uid · i_gid', 'owner'), ('i_u', 'raw_blkaddr'), ('i_mtime', 'extended only')]
...
s3 = d.box('addr = (raw_blkaddr + blk) × 4 KiB + X mod 4 KiB', 830, 360, 440, 60, 'green', sub='in data area')
...
d = Diagram()
d.text('Directory\ninode slot', 20, 60, 120, 70, bold=True)
ino = d.box('erofs_inode', 170, 60, 220, 70, 'yellow', sub='S_IFDIR · i_size = dir bytes')
...
d.edge(ino, b0, (.5, 0), (.5, 0), 'i_u.raw_blkaddr · blocks globally sorted by name', points=[(280, 25), (875, 25)], offset=(0, -6), lstyle='verticalAlign=bottom;')
...
DF = [('nid', 430, 150, '8 B · inode id'), ('nameoff', 590, 120, '2 B · name start'), ('file_type', 720, 120, '1 B · DT_DIR …'), ('reserved', 850, 100, '1 B')]
...
l5 = d.box('inode @ meta_blkaddr × 4 KiB + nid × 32', 1200, 480, 320, 70, 'orange', sub='read erofs_inode · recurse per path component')
